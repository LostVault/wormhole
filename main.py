...
async def ping(ctx, amount=1):
    ...
    await ctx.channel.purge(limit=amount)
    # Отправляем сообщение и удаляем его через 13 секунд
    await ctx.send(f'{ctx.author.mention} pong! ({client.latency*1000}ms)', delete_after=13)
# ------------- КОММАНДА ПРОВЕРКА ПРИЛОЖЕНИЯ // КОНЕЦ


# ------------- УДАЛЕНИЕ СООБЩЕНИЙ НА КАНАЛЕ
...
async def clear(ctx, amount=100):
    ...
    await ctx.channel.purge(limit=amount)
# ------------- УДАЛЕНИЕ СООБЩЕНИЙ НА КАНАЛЕ // КОНЕЦ
...
async def shutdown(ctx, amount=1):
    ...
    await ctx.channel.purge(limit=amount)
    # Отправляем сообщение и удаляем его через 13 секунд
    await ctx.send(f'За что {ctx.author.mention}, мы же так хорошо общались!', delete_after=13)
